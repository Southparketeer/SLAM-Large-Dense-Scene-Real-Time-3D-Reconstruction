...
//				const Ti p = dIn(x,y) / 1000.;
//				float sum = 0;
//				float sumw = 0;
//
//				for(int r = -size; r <= size; ++r ) {
//					for(int c = -size; c <= size; ++c ) {
//						const Ti q = dIn.GetWithClampedRange(x+c, y+r) / 1000.;
...
namespace loo
{
	...
	template __declspec(dllexport) void BilateralFilter(Image<float>, const Image<float>, float, float, uint, float);
	template __declspec(dllexport) void BilateralFilter(Image<float>, const Image<unsigned short>, float, float, uint, unsigned short);
	...
	__global__ void KernBilateralFilter( Image<To> dOut, const Image<Ti> dIn, const Image<Ti2> dImg, float gs, float gr, float gc, int size) 
	{
		const uint x = blockIdx.x*blockDim.x + threadIdx.x;
		const uint y = blockIdx.y*blockDim.y + threadIdx.y;
	...
	template __declspec(dllexport) void BilateralFilter(Image<float>, const Image<float>, const Image<unsigned char>, float, float, float, uint);
	template __declspec(dllexport) void BilateralFilter(Image<float>, const Image<float>, const Image<float>, float, float, float, uint);
}
