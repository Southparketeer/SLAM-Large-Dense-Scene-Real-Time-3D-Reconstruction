...
namespace loo
{
	...
	__global__ void KernMarchingCubeMaskGPU(BoundedVolume<SDF_t> vol, BoundedVolume<uchar2> vol_mask, float trunc_dist)
	{
		...
		for(int iVertex = 0 ; iVertex < 8 ; iVertex ++)
		{
			...
			if(afCubeValue[iVertex] >= 0.9 * (trunc_dist)) 	{ vol_mask(x,y,z) = make_uchar2(0,0);	return;	}
			if(afCubeValue[iVertex] <= -0.9 * (trunc_dist)) { vol_mask(x,y,z) = make_uchar2(0,0);	return;	}
		}
		...
		for(int iTriangle = 0; iTriangle < 5; iTriangle++)
		{
			...
			for(int iCorner = 0; iCorner < 3; iCorner++)
			{
				...
				if(dot(asEdgeNorm[iVertex], make_float3(0,0,-1))<= -0.1)
				{
					no_face = true;
					break;
				}
	...
	void MarchingCubeMaskGPU(BoundedVolume<loo::SDF_t> vol, BoundedVolume<uchar2> vol_mask, float trunc_dist )
	{
		dim3 blockDim(8,8,8);
		dim3 gridDim(vol.w / blockDim.x, vol.h / blockDim.y, vol.d / blockDim.z);
		KernMarchingCubeMaskGPU<<<gridDim,blockDim>>>(vol, vol_mask, trunc_dist);
		//GpuCheckErrors();
	}
}
