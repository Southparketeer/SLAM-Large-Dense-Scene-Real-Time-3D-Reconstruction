	...
	void vMarchCube(
		...
		float truncate_dist = 100.f
			...
			for(int iVertex = 0; iVertex < 8; iVertex++) {
				...
				if(afCubeValue[iVertex] >= (truncate_dist * 0.9f) || afCubeValue[iVertex] <= -(truncate_dist * 0.9f)) return;
			}
			...
			for(int iTriangle = 0; iTriangle < 5; iTriangle++)
			{
				...
				if(!no_face)
				{
					...
					for(int iCorner = 0; iCorner < 3; iCorner++)
					{
						...
						//norms.push_back(make_float3(asEdgeNorm[iVertex].x,   asEdgeNorm[iVertex].y,  asEdgeNorm[iVertex].z) );
		...
		for(GLint iX = 0; iX < vol.Voxels().x-1; iX++) {
			for(GLint iY = 0; iY < vol.Voxels().y-1; iY++) {
				for(GLint iZ = 0; iZ < vol.Voxels().z-1; iZ++) {
					vMarchCube(vol, volColor, iX,iY,iZ, verts, faces, iso, truncate_dist);
				}
			}
		}
